...
window.submitCheckup = async function(){
    ...
    const body={
        ...
        tanggal_pemeriksaan: document.getElementById('c_tgl_periksa')?.value,
    ...
    for(const cb of checked){
        await fetch(BASE_URL+'/imunisasi',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+token},body:JSON.stringify({anak_id:anakId,nama_vaksin:cb.value,tanggal_jadwal:document.getElementById('c_tgl_periksa')?.value})}).then(r=>r.json()).catch(()=>null);
    }
...
function injectModals(){ document.body.insertAdjacentHTML('beforeend',anakModalHTML); document.body.insertAdjacentHTML('beforeend',imunisasiModalHTML); }
